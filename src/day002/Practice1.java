package day002; // 현재 클래스가 위치한 폴더(패키지)명

import java.util.Scanner;

public class Practice1 { //class start
    // m + enter, ,자바 실행은 main함수(스레드)로 부터 (실행)흐름 시작된다.
    public static void main(String[] args) { // main start (메인 함수부터 실행된다.)

        // 문제 1
        // 리터럴: 개발언어에서 미리 정해진 값들을 = 상수 = 키보드로부터 입력
        // sout + enter
        System.out.println( 25 );
        System.out.println( "홍길동" ); // "문자열" '문자' (`백틱없음`)

        // 문제 2
        // 변수란?: 하나의 자료를 저장하는 메모리(저장소) 공간
        // 타입 변수명 = 값;
        int age1 = 10; // 타입: int , 변수명: age , 값: 10. ;: 자바는 문장마다 끝에 ;세미콜론 넣는다.
        System.out.println( age1 ); // "age"는 age 문자. age는 age라는 키워드.
                                   // 문자열: 문자 그 자체 , 키워드: 프로그램이 이해하는 단어 (기능 포함)(변수명/함수명/if/for/int등)

        // 문제 3
        // 변수는 동일한 { } 내 중복 이름이 불가능하다.
        String name = "이름";
        int age2 = 10;
        double height = 180.2;
        String motto = "좌우명";
        ...
        // 문제 4
        // + 더하기 기호는 2가지 기능 갖는다.
        // 1) 더하기 기호 (숫자+숫자) , 2) 연결 (숫자+문자)
        System.out.println( "제 이름은 " + name + "이고, 나이는 " + age2 + "세, 키는 " + height + "cm 입니다." );

        // 문제 5 printf형식
        // 문자열 내 복잡한 수식이나 변수가 들어가는 자리에 형식으로 표현
        // %d 정수 %f 실수 %s 문자열 %c 문자
            // 몰라서찾아봄) 소수점 자릿수: %전체자릿수.소수점자릿수f <- (소수점포함)전체 자릿수 만큼 자릿수 차지.
        System.out.printf( "제 이름은 %s 이고, 나이는 %d 세, 키는 %.1f cm 입니다. \n", name , age2 , height );
        ...
        System.out.printf("%-3s %-5s %-10s \n" , "번호", "작성자", "방문록");  // %3s: 3자리를 차지하는 문자열 자리, 오른쪽 정렬
        System.out.printf("%-3s %-5s %-10s \n", num, writer, content);      // %-3s: 왼쪽 정렬
        System.out.println("===============================\n");

        /* 문제 8: Scanner를 이용하여 사용자의 이름(String)과 나이(int)를 입력받아,
                  "OOO님의 나이는 OO세 입니다." 형식으로 출력하는 코드를 작성하시오.
        입력 예시:
        이름을 입력하세요: 유재석
        나이를 입력하세요: 51
        출력 예시:
        유재석님의 나이는 51세 입니다.*/
        Scanner scan = new Scanner(System.in); // 입력받기 준비물!
        System.out.print("8. 이름: ");
        String 이름 = scan.next(); // .next() console에서 입력받은 자료를 문자열로 반환
        System.out.print("8. 나이: ");
        int 나이 = scan.nextInt(); // .nextInt() console에서 입력받은 자료를 정수로 반환
        ...
        제목: 자바는 재미있어요
        내용: 정말입니다. 다들 동의하시죠?*/
        System.out.print("9-1. 게시물번호: ");        int bno = scan.nextInt();
        scan.nextLine(); // 하나 추가해줘야 한다.
        System.out.print("9-2. 제목: ");        String title = scan.nextLine();
        System.out.print("9-3. 내용: ");        String comment = scan.nextLine();
        ...
        출력 예시:
        입력하신 성별은 '남' 입니다.*/
        // 문자 입력받기: next().charAt(0) (nextChar는 없다.)
        System.out.print("10. 성별을 입력하세요(남/여): ");
        char 성별 = scan.next().charAt(0); // (0)은 첫 글자 입력받기. 입력받은 첫글자만 반환.
